...
pdf.bounding_box [150, 650], :width => 100, :height => 150 do
    pdf.fill_color colors[:black]
    pdf.text @current_facture.ref , :align => :left
    pdf.move_down(5)
    pdf.text @current_facture.created_at.to_date.to_s(), :align => :left
    pdf.move_down(5)
    pdf.text @current_facture.statut, :align => :left
end
...
if @current_facture.facture_type == 'calcul'
  ...
  pdf.bounding_box [160, 480], :width => 200, :height => 20 do
      pdf.text  @current_facture.credit.forfait.name , :align => :left 
  end
  ...
  pdf.bounding_box [230, 465], :width => 100, :height => 45 do
      pdf.text @current_facture.credit.forfait.nb_jetons.to_s(), :align => :left, :size => 10
      pdf.text @current_facture.credit.forfait.nb_jetons_tempon.to_s(), :align => :left, :size => 10
      pdf.text @current_facture.credit.forfait.validity.to_s(), :align => :left, :size => 10
  end

  # on insere les chiffres
  pdf.fill_color colors[:blue]
  pdf.bounding_box [360, 480], :width => 70, :height => 20 do
      pdf.text  @current_facture.price_calcul_HT.to_s() , :align => :center
  end
  pdf.bounding_box [430, 480], :width => 70, :height => 20 do
      pdf.text  @current_facture.price_calcul_TVA.to_s(), :align => :center
  end
  pdf.bounding_box [500, 480], :width => 70, :height => 20 do
      pdf.text  @current_facture.price_calcul_TTC.to_s(), :align => :center
  end
...
# si le type de facture est un memoire ----------------------------------------------------------------
elsif @current_facture.facture_type == 'memoire'
  # on insere les noms des lignes
  pdf.fill_color colors[:blue]
  ...
  pdf.bounding_box [250, 480], :width => 120, :height => 20 do
      pdf.text  @current_facture.log_abonnement.abonnement.name , :align => :left 
  end
  ...
  pdf.bounding_box [230, 465], :width => 100, :height => 45 do
      pdf.text @current_facture.log_abonnement.abonnement.assigned_memory.to_s(), :align => :left, :size => 10
      pdf.text @current_facture.log_abonnement.abonnement.security_level.to_s(), :align => :left, :size => 10
      pdf.text @current_facture.log_abonnement.abonnement.nb_max_user.to_s(), :align => :left, :size => 10
  end

  # on insere les chiffres
  pdf.fill_color colors[:blue]
  pdf.bounding_box [360, 480], :width => 70, :height => 20 do
      pdf.text  @current_facture.price_memory_HT.to_s() , :align => :center
  end
  pdf.bounding_box [430, 480], :width => 70, :height => 20 do
      pdf.text  @current_facture.price_memory_TVA.to_s(), :align => :center
  end
  pdf.bounding_box [500, 480], :width => 70, :height => 20 do
      pdf.text  @current_facture.price_memory_TTC.to_s(), :align => :center
  end
...
pdf.bounding_box [360, 410], :width => 70, :height => 20 do
    pdf.text  @current_facture.total_price_HT.to_s() , :align => :center, :style => :bold
end
pdf.bounding_box [430, 410], :width => 70, :height => 20 do
    pdf.text  @current_facture.total_price_TVA.to_s(), :align => :center, :style => :bold
end
pdf.bounding_box [500, 410], :width => 70, :height => 20 do
    pdf.text  @current_facture.total_price_TTC.to_s(), :align => :center, :style => :bold
end
