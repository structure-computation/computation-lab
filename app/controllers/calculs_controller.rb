class CalculsController < ApplicationController
  ...
  def index
    ...
    current_model = current_company_member.sc_models.find(@model_id)
  ...
  def new
    ...
    @current_calcul.company_member = current_company_member
  ...
  def get_brouillon
    @current_model  = current_company_member.sc_models.find(params[:id_model])
    @current_calcul = @current_model.calcul_results.find(params[:id_calcul])
    send_data       = @current_calcul.get_brouillon(params,current_company_member)
  ...
  def load_brouillon_from_ext_file
    ...
    results = @current_calcul.load_brouillon_from_ext_file(params,current_company_member)
    if !results
