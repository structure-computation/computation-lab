class DetailModelController < ApplicationController
  ...
  def send_mesh
    current_model = current_workspace_member.sc_models.find(params[:id_model])
    results = current_model.send_mesh(params,current_workspace_member) 
  ...
  def send_new_file
    current_model = current_workspace_member.sc_models.find(params[:id_model])
    results = current_model.send_file(params,current_workspace_member)
    # envoie de la reponse au client
    render :text => results
  end
